...
class UserRowModel(UserModel):
    ...
    role: list[RoleModel] = Field(default_factory=list, description='角色信息')
...
class UserDetailModel(BaseModel):
    ...
    posts: list[PostModel | None] = Field(default=[], description='岗位信息')
...
class ChangeVipModel(BaseModel):
    """
    修改用户VIP授权模型
    """
...
class AiRecognitionCountModel(BaseModel):
    """
    修改用户AI识图剩余次数模型
    """
