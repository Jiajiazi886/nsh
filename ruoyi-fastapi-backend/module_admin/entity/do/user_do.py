...
class SysUser(Base):
    ...
    vip_ai_image_recognition_count = Column(Integer, nullable=False, server_default='0', comment='VIP AI识图剩余次数')
    sponsor_enabled = Column(CHAR(1), nullable=False, server_default='0', comment='赞助开关（0关闭 1开启）')
    sponsored_vip = Column(CHAR(1), nullable=False, server_default='0', comment='赞助VIP标识（0非VIP 1VIP）')
    sponsored_by_user_id = Column(BigInteger, nullable=True, comment='赞助来源帮会管理用户ID')
...
# SysUserPost 已废弃 — sys_user_post 表已删除（帮会系统不需要岗位功能）
