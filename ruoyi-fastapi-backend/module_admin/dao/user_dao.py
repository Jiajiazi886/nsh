...
class UserDao:
    ...
    @classmethod
    async def get_roles_by_user_ids(cls, db: AsyncSession, user_ids: list[int]) -> dict[int, list[SysRole]]:
        if not user_ids:
            return {}

        query = (
            select(SysUserRole.user_id, SysRole)
            ...
            .order_by(SysRole.role_sort, SysRole.role_id)
        )
        rows = (await db.execute(query)).all()
        role_map: dict[int, list[SysRole]] = {}
